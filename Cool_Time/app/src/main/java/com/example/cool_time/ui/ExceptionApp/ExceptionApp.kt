...
class ExceptionApp : Fragment(){
    override fun onCreateView(
        ...
        val uncheckedadapter = AppAdapter(uncheckeddatas, binding.uncheckedException, checkeddatas)
        binding.checkedException.adapter = checkedadapter
        binding.uncheckedException.adapter = uncheckedadapter



        uncheckeddatas.addOnListChangedCallback(object: ObservableList.OnListChangedCallback<ObservableList<AppItem>>(){
            override fun onChanged(sender: ObservableList<AppItem>?) {
                uncheckedadapter.notifyDataSetChanged()
            }

            override fun onItemRangeChanged(sender: ObservableList<AppItem>?, positionStart: Int, itemCount: Int) {
                // 특정 범위 내의 항목이 변경될 때 수행할 작업
                uncheckedadapter.notifyDataSetChanged()
            }

            override fun onItemRangeInserted(sender: ObservableList<AppItem>?, positionStart: Int, itemCount: Int) {
                uncheckedadapter.notifyDataSetChanged()
            }

            override fun onItemRangeMoved(sender: ObservableList<AppItem>?, fromPosition: Int, toPosition: Int, itemCount: Int) {
                uncheckedadapter.notifyDataSetChanged()
            }

            override fun onItemRangeRemoved(sender: ObservableList<AppItem>?, positionStart: Int, itemCount: Int) {
                uncheckedadapter.notifyDataSetChanged()
            }
        })

        checkeddatas.addOnListChangedCallback(object: ObservableList.OnListChangedCallback<ObservableList<AppItem>>(){
            override fun onChanged(sender: ObservableList<AppItem>?) {
        ...
        binding.uncheckedException.layoutManager  = LinearLayoutManager(this.activity, LinearLayoutManager.VERTICAL, false)
        binding.checkedException.layoutManager  = LinearLayoutManager(this.activity, LinearLayoutManager.VERTICAL, false)
        return binding.root
    }
}